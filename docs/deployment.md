...
**Важно про пробелы.** Frigate срезает пробелы у секрета из `.jwt_secret`, но **не** срезает у `FRIGATE_JWT_SECRET` из окружения. Скопированный из файла перевод строки невидим в `.env` и меняет подпись целиком — со стороны выглядит как «секрет тот же, а не работает». Мы свои значения срезаем; проверить чужие поможет команда ниже.
