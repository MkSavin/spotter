...
1. В упавшем прогоне нажми **Re-run failed jobs** — пересоберутся только упавшие
   джобы, уже собранные образы трогать не нужно. Когда все станут зелёными,
   автоматически отработает джоба `publish`: проставит теги и создаст Releases.
...
> поэтому перезапуск прогона (или новый push в `master`) снова возьмётся за сборку.
> Как только теги появились, релиз считается состоявшимся и повторных сборок не будет.
