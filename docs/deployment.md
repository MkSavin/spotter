...
`FRIGATE_URL` — любой адрес, до которого достаёт контейнер адаптера: имя контейнера в общей сети (`http://frigate:5000`), IP машины в локальной сети (`http://192.168.1.10:8971`), домен снаружи. Наружу этот адрес не отдаётся: получателям уходят presigned-ссылки на S3.

Держать Frigate на другой машине в локальной сети — штатный случай. Открывать его в интернет для этого не нужно, но если на нём включён `auth`, читай следующий раздел.

Прежнее имя `FRIGATE_REMOTE_URL` продолжает работать.
...
**Порт и протокол.** Аутентифицированный API — это `8971`, и он говорит **по HTTPS** с сертификатом, который Frigate выпускает сам. То есть `FRIGATE_URL=https://frigate:8971`, а не `http://`. Порт `5000` — тот же API, но вообще без проверки токена; он годится, только если закрыт от всех, кроме адаптера.

Самоподписанный сертификат надо разрешить явно:

```
FRIGATE_TLS_INSECURE=true
```

Без этого запросы падают с `self signed certificate`. Если Frigate стоит за прокси с настоящим сертификатом — флаг не нужен.

**Про пробелы.** Frigate срезает пробелы у секрета из `.jwt_secret`, но **не** срезает у `FRIGATE_JWT_SECRET` из окружения. Скопированный из файла перевод строки невидим и меняет подпись целиком — выглядит как «секрет тот же, а не работает». Проверено: секрет с лишним `\n` даёт ровно 401.

**Пользователь должен существовать.** При `auth.enabled: true` и пустой базе Frigate пишет в лог `Auth is enabled, but no users exist` — создай пользователя в его веб-интерфейсе, иначе `FRIGATE_AUTH_USER` не с чем сопоставить.
