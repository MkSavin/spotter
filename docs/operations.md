...
Сервисы обновляются не одновременно, поэтому сообщение отправляется через
полторы минуты после последней замеченной смены версии — вся волна попадает в
одно сообщение.

Версии хранятся в базе бота, так что его собственный перезапуск ничего не
шлёт, а сервис, обновившийся пока бот лежал, не потеряется.
