...
```bash
./spotter watchtower          # показать текущий интервал
./spotter watchtower 3600     # раз в час
```

Значение записывается в `.env` и действует дальше само. Пересоздаётся только
контейнер watchtower — приложения не перезапускаются, событие в обработке не
прервётся.
