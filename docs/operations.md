...
```bash
./spotter recreate --watchtower-interval=3600   # раз в час
```

Значение записывается в `.env`, так что дальше действует и без флага. Нужен
именно `recreate`: интервал передаётся аргументом контейнера и фиксируется при
его создании, обычный `up` его не подхватит.

Сколько ставить: обновления приезжают только после релиза, поэтому чаще пары
часов смысла нет. Сутки — разумно, если не ждёшь конкретной правки; час — когда
выкатываешь и хочешь увидеть результат в тот же день. Меньше 15 минут не стоит:
watchtower на каждой проверке ходит в реестр за манифестами всех образов.
