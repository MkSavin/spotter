...
Транскодинг — самая требовательная часть, и `depot` рассчитан на размножение: состояния он не держит, поэтому несколько экземпляров уже делят работу через одну consumer-группу Redis. Разнести их по машинам и добавлять мощности под нагрузку — план в [спеке масштабирования depot](docs/depot-scaling.md).
