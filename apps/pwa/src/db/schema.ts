...
/**
 * An authorized install. Kept apart from `push_subscriptions` on purpose: a
 * device is authorized by redeeming a code, which has nothing to do with
 * whether it later allows notifications — and browsers rotate or drop a push
 * endpoint without the user doing anything.
 *
 * `token` is the bearer the client presents; `recipient_uuid` and `role` are
 * what the domain granted, cached here so a request need not ask every time.
 */
export const devices = sqliteTable('devices', {
  /** Stable per install, generated by the client. */
