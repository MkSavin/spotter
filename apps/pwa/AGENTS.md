...
Код можно ввести руками или прийти по ссылке `/authorize?code=…`, которую `/user_sign` кладёт в сообщение. Ссылка **гасится сразу**: код подставляется, отправляется и вычищается из адресной строки через `replaceState` — он одноразовый, и ему нечего делать в истории, закладке или скриншоте. Если установка уже авторизована, `LoginPage` не рендерится вовсе, поэтому код из URL там убирает [App.tsx](src/../web/src/App.tsx) — иначе он остался бы висеть в адресе.

Отдельного маршрута `/authorize` нет: страница его потребляет и заменяет адрес, так что для роутера это обычный неизвестный путь.

Свой адрес PWA объявляет **в heartbeat** (`details.url` из `PUBLIC_URL`), и бот берёт ссылку оттуда, а не из своего конфига — иначе они разъехались бы при первом же переезде.
...
| `GET /api/timelapses` | authorized | собранные и собирающиеся таймлапсы |
| `POST /api/timelapses` | USER | запустить сборку |
| `GET /api/users` | ADMIN | кто имеет доступ |
| `POST /api/users/{role,revoke,sign}` | ADMIN | роль, отзыв, выдача кода |
...
Экспорт живёт минутами и переживает рестарт, поэтому запущенные лежат в таблице
`timelapses`, а не в памяти. Сообщение `timelapse.ready` не несёт id запроса — корреляция
идёт по `camera:start:end`, и этот же ключ служит id строки, так что повторная доставка
обновляет её, а не плодит дубль. Экспорт, доехавший после рестарта (запроса уже никто не
ждёт), всё равно записывается: видео существует, терять его незачем.

`timelapse.failed` приходит без периода — только камера и причина, — поэтому помечается
всё, что у этой камеры ещё `running`. Уже готовый экспорт при этом не трогается.

Скоростей две, потому что столько принимает API Frigate; кратность быстрой задаётся в
конфиге NVR, и UI её не обещает. Подробности — в [apps/frigate/AGENTS.md](../frigate/AGENTS.md).

## Управление пользователями

Все маршруты просто пересылают команду домену (`user.list`/`setRole`/`revoke`/`sign`) и
возвращают его ответ — PWA не пишет в доменные таблицы. Проверка ADMIN здесь удобство:
сервер перепроверяет роль по `recipientUuid`.

Отозвать самого себя нельзя — последний админ иначе запрёт себя снаружи домена.

`findByRef` на сервере теперь принимает и uuid: у recipient'а, созданного из PWA, нет ни
`tgUserId`, ни username, и без этого им нельзя было бы управлять.
