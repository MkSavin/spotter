...
- 3563382: Добавлен кулдаун на команды: повтор в том же чате отсекается (3 с по умолчанию, 60 с для `/timelapse`). Команды, читающие только локальное состояние, не ограничиваются
...
- 8d537a3: Исправлена длительность событий дольше суток: копия `renderEventTiming` считала часы по модулю 60 и показывала «1 дней 25 ч»
