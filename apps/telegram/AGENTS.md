...
Сервисы шлют heartbeat в `spotter.heartbeat` раз в 30 секунд; `heartbeatController` раздаёт их трём потребителям:
...
- `SourceWatcher` — раз в минуту проверяет, жив ли NVR за адаптером, и шлёт админам тревогу **со звуком** при обрыве и беззвучное сообщение при восстановлении. По таймеру, а не по приходу удара: сломавшийся NVR не пришлёт сообщение о том, что он сломался, — ожидание такого сообщения в сентябре 2026 стоило двух суток тишины. Подробности — [docs/source-watchdog.md](../../docs/source-watchdog.md).

Адаптеры добавляют в удар `source`: когда было последнее событие, когда NVR **вообще что-либо** сказал (`lastContactAt`), и что он сам думает о своих камерах. Два сигнала намеренно разные: тишина по событиям нормальна ночью и терпит 6 часов, а отсутствие служебного трафика не бывает нормой и ловится за 15 минут.

В том же ударе едет глубина очередей (`queues`): `lag` — сколько записей никому не выдано, `pending` — взято и не подтверждено, `oldestPendingMs` — возраст старейшей незакрытой. `/status` печатает её только когда есть что печатать: пустая очередь — норма, и строка нулей на каждом сервисе похоронила бы единственное число, которое важно. Растущий `lag` значит «не успеваем», растущий `oldestPendingMs` — «обработчик застрял»; это разные диагнозы.
