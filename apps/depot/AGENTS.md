...
- **Транзиентный** сбой (S3 недоступен, ключ ещё не виден) — это `TransientError`, и он
  **пробрасывается наружу**: регулятор не делает `XACK`, запись остаётся в PEL, reaper
  ...
  медиа навсегда.
- **Таймаут ffmpeg — не транзиентный.** Он означает зависший процесс, а не длинный клип:
  длина больше ни во что не упирается. Повтор упрётся в тот же `VIDEO_TIMEOUT_MS` (4 часа),
  поэтому упавшее задание ждёт следующего старта, а не немедленного ретрая.
...
- У depot **нет доступа к БД**; единственное состояние — очередь принятых заданий
  (`TRANSCODE_STATE_PATH`, том `/data`), всё остальное через Redis Streams и S3.
- depot **не знает NVR**: на вход приходят только S3-ключи застейдженного сырья (никаких URL/токенов).
- **Транскодирование живёт вне Redis-записи.** `mediaStagedController` кладёт задание в
  [TranscodeQueue](src/jobs/TranscodeQueue.ts) и сразу возвращается, поэтому `XACK` проходит за
  миллисекунды, а ffmpeg работает сколько нужно. `REDIS_RECLAIM_MIN_IDLE_MS` снова означает
  «реплика умерла» и не обязан превышать длительность конвертации.
- Плата за это — **очередь обязана переживать рестарт**: после `XACK` запись никто не
  перевыдаст. Задание пишется в стор до запуска, снимается после успеха и остаётся при сбое;
  `recover()` на старте забирает незаконченное. Том `/data` для этого обязателен.
