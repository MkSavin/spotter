...
по ключу, транскодит (ffmpeg / `Bun.Image`), кладёт результат обратно в S3 и отвечает ключами.
...
(GPU-оверлей резервирует карту только клиповым: снапшот-лейн работает через `Bun.Image`, без ffmpeg).
...
- **Транзиентный** сбой (S3 недоступен, ключ ещё не виден) — это `TransientError`, и он
  **пробрасывается наружу**: регулятор не делает `XACK`, запись остаётся в PEL, reaper
  повторит. Не заворачивай такие ошибки в `catch` — проглоченная ошибка означает потерю
  медиа навсегда.
- **Таймаут ffmpeg — не транзиентный.** Клип слишком длинный или машина слишком слабая:
  повтор упрётся в тот же `VIDEO_TIMEOUT_MS`, сожжёт бюджет доставок и на всё это время
  займёт воркер. Лечится увеличением таймаута (держи ниже `REDIS_RECLAIM_MIN_IDLE_MS`),
  а не ретраем.
...
- **Изображения:** `Bun.Image` — встроен в рантайм, нативных бинарников не требует. JPEG
  кодируется с `progressive: true`: у Bun нет `optimiseCoding`, и без него тот же quality даёт
  файл примерно на 18% тяжелее.
